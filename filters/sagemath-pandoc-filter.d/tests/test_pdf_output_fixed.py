...
class TestPDFOutput(SageTestBase):
    ...
    def run_pandoc_with_filter(self, md_content, output_pdf):
        """Helper method to run pandoc with the sagemath-pandoc-filter."""
        try:
            # Write markdown content to a file in the test directory
            ...
            filter_path = os.path.join(
                ...
                'sagemath_pandoc_filter', 'filter.py'
            )
            ...
            cmd = [
                'pandoc',
                '--filter', sage_python, filter_path,
                '-o', output_pdf,
                md_path
            ]
            ...
            try:
                ...
                return False, f"Pandoc failed with error: {e.stderr}"
...
"""
            ...
            success, error = self.run_pandoc_with_filter(md_content, output_pdf)
            
            if not success:
                self.fail(f"Pandoc failed with error: {error}")
            
            # Check that PDF was created
            self.assertTrue(os.path.exists(output_pdf), 
                          f"PDF file not created at {output_pdf}")
            
            # Verify the PDF is not empty
            self.assertGreater(os.path.getsize(output_pdf), 1000, 
                             f"PDF file is too small: {output_pdf}")
            
            # If pypdf is available, verify the PDF content
            if HAS_PYPDF:
...
"""
            ...
            # If pypdf is available, verify the PDF contains the plot
            if HAS_PYPDF:
                with open(output_pdf, 'rb') as f:
                    reader = pypdf.PdfReader(f)
                    text = "\n".join(page.extract_text() for page in reader.pages)
                    self.assertIn('Parabola', text, "Plot title not found in PDF")
            
            # Check if the plot image was created in the sage_images directory
            plot_files = [f for f in os.listdir(self.sage_images_dir) if f.endswith('.png')]
            self.assertGreater(len(plot_files), 0, "No plot images were generated")
